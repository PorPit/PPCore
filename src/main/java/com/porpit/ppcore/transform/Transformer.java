...
public abstract class Transformer {
	...
	public void replaceLabel(InsnList instructions, AbstractInsnNode node,
			@Nullable ArrayList<AbstractInsnNode> replaceInstructions, int labels, boolean keepFirstLabel) {
		replaceLabelBefore(instructions, node, replaceInstructions, labels, 0, keepFirstLabel, true);
	}

	public void replaceLabelBefore(InsnList instructions, AbstractInsnNode node,
			@Nullable ArrayList<AbstractInsnNode> replaceInstructions, int labels, int labelsBefore,
			boolean keepFirstLabel, boolean deleteFrame) {
		ListIterator<AbstractInsnNode> iterator = instructions.iterator();
	...
	public static String patchMethodName(String className,String methodName, String desc) {
		return TransformerNames.patchMethodName(methodName, desc, patchClassName(className.replace(".", "/")));
	}
	...
	public MethodNode findMethod(ClassNode node, String name, String desc,boolean needPatch) {
		if(needPatch==true){
			return findMethod(node,name ,desc );
		}else {
			Iterator<MethodNode> methods = node.methods.iterator();
			while (methods.hasNext()) {
				MethodNode m = methods.next();
